...
- **Asistente Virtual AI** con Google Gemini: chat multimodal, análisis de imágenes y comandos de voz (STT/TTS).
...
| `VITE_GEMINI_API_KEY` | API Key de Google Gemini para el módulo AI Agent (ver `AI_AGENT_SETUP.md`) |
...
- **`client/src`**
  ...
  - `pages/`: login, registro, recuperación, dashboard, perfil, chats, biblioteca, grupos, proyectos, moderación, ai-agent.
  - `services/`: cliente Axios con refresh automático de JWT, servicio de Gemini AI.
...
El proyecto incluye un asistente virtual multimodal usando Google Gemini API. Para configurarlo, consulta el archivo [`client/AI_AGENT_SETUP.md`](client/AI_AGENT_SETUP.md).

**Características:**
- Chat de texto con historial de contexto
