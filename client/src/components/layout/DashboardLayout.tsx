...
import { Home, Library, Users, FolderKanban, LogOut, Shield, Compass, MessageCircle, Sparkles } from 'lucide-react';
          ...
          <nav className="flex justify-center">
            <div className="flex items-center gap-1 overflow-x-auto rounded-full px-2.5 py-1.5 sm:gap-1.5 sm:px-3">
              {navigation.map(({ to, label, icon: Icon }) => {
                const isAiAgent = to === '/ai-agent';
                // Debug: verificar que el botón AI esté en el array
                if (isAiAgent) {
                  console.log('[DashboardLayout] AI Agent button found in navigation:', { to, label });
                }
                return (
                  <NavLink
                    key={`nav-${to}`}
                    ...
                    className={({ isActive }) =>
                      classNames(
                        'flex items-center gap-1 rounded-2xl px-2 py-1 text-[10px] font-semibold transition-all sm:gap-1.5 sm:px-3 sm:py-1.5 sm:text-[11px] relative',
                        isActive
                        ...
                        isAiAgent && !isActive && 'hover:bg-sena-green/10'
                      )
                    }
                  >
                    <span
                      className={classNames(
                        'flex h-6 w-6 items-center justify-center rounded-2xl shadow-[0_6px_12px_rgba(18,55,29,0.16)] sm:h-7 sm:w-7',
                        isAiAgent
                          ? 'bg-gradient-to-br from-sena-green/90 to-emerald-400/80 text-white'
                          : 'bg-white/40 text-sena-green'
                      )}
                    >
                    ...
                    <span>{label}</span>
                    {isAiAgent && (
                      <span className="absolute -top-1 -right-1 h-2 w-2 rounded-full bg-sena-green animate-pulse" />
                    )}
                  </NavLink>
                );
              })}
