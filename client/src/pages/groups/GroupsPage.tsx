    ...
    <DashboardLayout title="Grupos" subtitle="Organiza comunidades y grupos de estudio para aprender juntos.">
      <div className="grid w-full gap-5 pb-16 lg:grid-cols-[minmax(0,280px)_minmax(0,1fr)] xl:grid-cols-[minmax(0,320px)_minmax(0,1fr)]">
        <aside className="hidden flex-col gap-4 lg:flex">
          <Card className="h-full bg-white/20 p-4 backdrop-blur-xl shadow-[0_12px_28px_rgba(18,55,29,0.15)] dark:bg-white/10">
            ...
            <div className="mt-3 space-y-3 overflow-y-auto pr-1">
              ...
              {activeGroups.map((group) => (
                <div key={group.id} className="rounded-2xl border border-white/20 bg-white/15 px-3 py-2">
          ...
          <Card className="bg-white/20 p-4 backdrop-blur-xl shadow-[0_12px_28px_rgba(18,55,29,0.15)] dark:bg-white/10">
        ...
        <section className="flex flex-col gap-5">
          <Card className="bg-white/25 p-5 backdrop-blur-xl shadow-[0_12px_28px_rgba(18,55,29,0.15)] dark:bg-white/10">
          ...
          <div className="flex flex-col gap-3">
            ...
            {isLoading ? (
              <Card className="bg-white/20 p-6 text-sm text-[var(--color-muted)] backdrop-blur-xl">Cargando grupos...</Card>
            ) : filteredGroups.length === 0 ? (
              <Card className="bg-white/20 p-6 text-sm text-[var(--color-muted)] backdrop-blur-xl">
                No se encontraron grupos. Intenta con otra palabra clave o crea uno nuevo.
              </Card>
            ) : (
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                {filteredGroups.map((group) => (
                  <Card key={group.id} className="flex h-full flex-col gap-3 bg-white/20 p-4 backdrop-blur-xl shadow-[0_10px_24px_rgba(18,55,29,0.14)]">
